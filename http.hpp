...
public:
  ...
  static int run_forever() {
    return event_base_dispatch(Event::event_base_global());
  }

  ///////////////////////////////////////////////////////////////////////
  // Schedule a callback
  template <class F, class... Args> static void call_soon(F f, Args... args) {
    struct wrapper {
    ...
    static event_callback_fn event_base_once_cb = [](evutil_socket_t fd,
                                                     short what, void *ptr) {
    ...
    event_base_once(
        Event::event_base_global(), -1, EV_TIMEOUT, event_base_once_cb,
...
void start_server(unsigned short port,
  ...
  public:
    void do_read_request() {
      static llhttp_cb llhttp_on_message = [](llhttp_t *req) -> int {
        ...
        auto do_write_ = [func_ = std::bind(&http_session::do_write_response,
                                            self, res)]() { func_(); };
      ...
      static bufferevent_event_cb bev_err_cb = [](struct bufferevent *bev,
                                                  short what, void *ctx) {
        void *arg = NULL;
    ...
    void do_write_response(response res) {
      ...
      static bufferevent_data_cb continue_read_ = [](bufferevent *bev,
                                                     void *arg) {
        internal_wrapper *wrapper_ = (internal_wrapper *)arg;
        auto self = wrapper_->p_session;
        auto do_read = [func_ = std::bind(&http_session::do_read_request,
                                          self)]() { func_(); };
  ...
  static evconnlistener_cb on_accept =
      [](struct evconnlistener *listener, evutil_socket_t fd,
         struct sockaddr *sa, int socklen, void *arg) {
        bufferevent *bev = bufferevent_socket_new(Event::event_base_global(),
                                                  fd, BEV_OPT_CLOSE_ON_FREE);

        std::tuple<std::function<void(response &)>> *p =
            (std::tuple<std::function<void(response &)>> *)arg;
  ...
  evconnlistener *listener =
      evconnlistener_new_bind(Event::event_base_global(), on_accept,
                              new std::tuple<decltype(_handler)>(_handler),
                              flags, -1, sa, sizeof(sockaddr_in));

  if (listener != NULL) {
    std::cout << "server has started on port: " << port << " using "
              << event_base_get_method(Event::event_base_global()) << std::endl;
    Event::run_forever();
  } else
